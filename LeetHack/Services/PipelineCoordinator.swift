...
final class PipelineCoordinator {
    ...
    private let codePanelDetector = CodePanelDetector()
    ...
    /// The most recently detected code panel, if any.
    private(set) var detectedCodePanel: DetectedCodePanel?
    
    private let logger = Logger(subsystem: "com.leethack", category: "Pipeline")
    ...
    /// Runs a focused, fast code-panel detection pass on the current frame.
    /// Only runs OCR + CodePanelDetector — no full pipeline.
    func detectCodePanel() {
        Task {
            guard let frame = appState.latestFrame, let image = frame.cgImage else {
                logger.warning("detectCodePanel: no frame available")
                return
            }
            
            logger.info("Running focused code panel detection")
            ...
            // Detect the code panel
            let panel = await codePanelDetector.detect(
                in: image,
                ocrBlocks: ocrBlocks,
                displayBounds: displayBounds
            )
            
            self.detectedCodePanel = panel
            
            if let panel {
                logger.info("Code panel: lang=\(panel.language ?? "?") conf=\(panel.confidence) lines=\(panel.codeLines.count)")
            } else {
                logger.info("No code panel detected")
            }
        }
    }
